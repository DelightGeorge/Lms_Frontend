...
// ── Luxury Feature Card ───────────────────────────────
const PremiumFeature = ({ icon, title, desc, index }) => (
...
const BottomNavItem = ({ to, icon, label, active }) => (
  <Link to={to} className={`flex flex-col items-center text-xs gap-1 transition-colors ${active ? "text-amber-600" : "text-slate-400"}`}>
    {icon}
...
const Home = () => {
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50/50 pb-24 sm:pb-0 overflow-x-hidden">
          ...
          <section>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
              <PremiumFeature
                ...
                desc="Curated by industry experts. Every course designed for real job market demand."
                index={0}
              />
              <PremiumFeature
                icon={<Award size={24} />}
                title="Certified Excellence"
                desc="Earn recognized credentials that employers trust and value worldwide."
                index={1}
              />
              <PremiumFeature
                icon={<Users size={24} />}
                title="Expert Instruction"
                desc="Learn from leaders actively working in their fields with decades of experience."
                index={2}
              />
              <PremiumFeature
                icon={<Clock size={24} />}
                title="Lifetime Access"
                desc="Learn at your pace with unlimited access to all course materials forever."
                index={3}
              />
              <PremiumFeature
                icon={<Globe size={24} />}
                title="Global Community"
                desc="Connect with 2M+ learners worldwide. Network and collaborate meaningfully."
                index={4}
              />
              <PremiumFeature
                icon={<Sparkles size={24} />}
                title="Premium Content"
                desc="New, curated courses weekly. Content refreshed by experts monthly."
                index={5}
              />
            </div>
          </section>
                ...
                {categories.map((cat, idx) => {
                  ...
                  return (
                    <Link
                      ...
                      className="group relative overflow-hidden rounded-2xl p-6 bg-gradient-to-br text-white hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 border border-white/10 h-32 flex flex-col justify-between"
                      style={{
                    ...
                    >
                      ...
                      <div className="relative z-10 flex flex-col justify-between h-full">
                        <div>
                          <BookOpen size={24} className="opacity-70 mb-2" />
                        </div>
                        <div>
                          ...
                          <p className="text-xs opacity-70 mt-1">
                            {courses.filter(
                              (c) =>
                                c.category?.name === cat.name && c.status === "PUBLISHED"
                            ).length}{" "}
                            courses
                          </p>
                        </div>
        ...
        <nav className="fixed bottom-0 inset-x-0 bg-white/95 backdrop-blur-md border-t border-slate-100 sm:hidden z-50 safe-area-pb">
          <div className="flex justify-around py-3 px-4">
            ...
            <BottomNavItem to="/courses" icon={<BookOpen size={20} />} label="Courses" active={location.pathname === "/courses"} />
            <BottomNavItem to="/categories" icon={<Sparkles size={20} />} label="Explore" active={location.pathname.startsWith("/categories")} />
