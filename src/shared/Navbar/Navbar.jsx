...
const Navbar = () => {
  ...
  const handleSearchChange = useCallback((val) => {
    ...
    if (!val.trim()) { setSearchResults({ courses: [], instructors: [] }); setShowDropdown(false); return; }
    searchTimerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const courses     = coursesRes.status === "fulfilled"     ? (coursesRes.value.data?.courses || coursesRes.value.data || []).slice(0,5)     : [];
        const instructors = instructorsRes.status === "fulfilled" ? (instructorsRes.value.data?.instructors || instructorsRes.value.data || []).slice(0,4) : [];
  ...
  const clearSearch = () => { setSearchQuery(""); setShowDropdown(false); setSearchResults({ courses: [], instructors: [] }); };
