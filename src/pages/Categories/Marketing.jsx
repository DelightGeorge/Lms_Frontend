  ...
  <Link to={`/courses/${course.id}`} className="group block">
    <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full flex flex-col">
      <div className="relative h-44 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
          <div className="w-14 h-14 bg-white/90 rounded-full flex items-center justify-center shadow-lg">
            <TrendingUp size={20} className="text-purple-600 ml-0.5 fill-current" />
          </div>
        </div>
        {course.price === 0 && (
        ...
        {course.price > 0 && (
          <span className="absolute top-4 left-4 bg-purple-500/90 backdrop-blur-sm text-white text-xs px-3 py-1.5 rounded-full font-bold border border-purple-400/30">
            ⭐ Premium
          </span>
        )}
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col">
        <h3 className="font-bold text-slate-900 line-clamp-2 mb-1.5 group-hover:text-purple-600 transition-colors text-sm">
          {course.title}
        </h3>
  ...
  useEffect(() => {
    getAllCourses()
      .then((r) => {
        ...
        const marketingCourses = all.filter(
          (c) =>
            c.status === "PUBLISHED" &&
            (c.category?.name?.toLowerCase() === "marketing" ||
              c.title?.toLowerCase().includes("marketing") ||
              c.title?.toLowerCase().includes("seo") ||
              c.title?.toLowerCase().includes("social media") ||
              c.title?.toLowerCase().includes("email"))
        );
        setCourses(marketingCourses.length > 0 ? marketingCourses : all.filter((c) => c.status === "PUBLISHED").slice(0, 8));
      })
      .catch(console.error)
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-purple-50 border border-purple-100">
              ...
              <span className="text-xs font-bold text-purple-600 uppercase tracking-widest">Marketing Expertise</span>
            </div>

            <h1 className="text-5xl lg:text-6xl font-black text-slate-900 leading-tight">
              Master <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600">Digital Marketing</span>
            </h1>

            <p className="text-lg text-slate-600 leading-relaxed max-w-xl">
              Learn strategies from top marketing professionals. From SEO and social media to email campaigns and analytics, boost your marketing career.
            </p>

            <div className="flex gap-4 flex-wrap pt-4">
              <div className="flex items-center gap-2 text-sm text-slate-700">
                <TrendingUp size={16} className="text-purple-500" />
                <span className="font-semibold">Real-world campaigns</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-slate-700">
          ...
          <div className="relative hidden lg:block">
            ...
            <div className="absolute -bottom-6 -left-6 bg-white p-4 rounded-2xl shadow-xl border border-slate-100">
              <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Marketing Skills</p>
              <p className="text-slate-900 font-black mt-1">Get Certified</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-white border border-slate-200 rounded-xl py-3 pl-12 pr-4 outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition"
            />
          </div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-slate-600 mt-1 text-sm">
            {searchTerm ? "Search results" : "Popular marketing courses"}
          </p>
        </div>
