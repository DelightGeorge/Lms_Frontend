...
export default function Pricing() {
  const plans = [
    ...
    },
    {
      ...
      to: "/for-businesses",
      features: [
        { text: "All course access",              ok: true  },
        { text: "Team progress dashboard",        ok: true  },
        { text: "Bulk certificates",              ok: true  },
    ...
    <Layout>
      <div className="min-h-screen bg-white pt-16">
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white py-20 px-4 text-center">
          <span className="inline-block text-xs font-bold text-amber-300 bg-amber-500/20 border border-amber-400/30 px-3 py-1.5 rounded-full mb-4 uppercase tracking-widest">Pricing</span>
          <h1 className="text-4xl sm:text-5xl font-black mb-4">Simple, transparent pricing</h1>
          <p className="text-slate-300 text-lg max-w-xl mx-auto">No subscriptions, no hidden fees. Pay once per course and own it forever.</p>
        </div>
        ...
        <div className="max-w-5xl mx-auto px-4 py-20">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {plans.map((plan) => (
              <div key={plan.name} className={`bg-white rounded-2xl border-2 ${plan.color} p-8 relative ${plan.popular ? "shadow-2xl shadow-blue-100" : "shadow-sm"}`}>
                {plan.popular && <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-xs font-black px-4 py-1 rounded-full">Most Popular</span>}
                ...
                <ul className="space-y-3 mb-8">
                  {plan.features.map(({ text, ok }) => (
                    <li key={text} className={`flex items-center gap-2 text-sm ${ok ? "text-slate-700" : "text-slate-300"}`}>
                      {ok ? <CheckCircle size={15} className="text-emerald-500 shrink-0" /> : <X size={15} className="text-slate-300 shrink-0" />}
                      {text}
                    </li>
                  ))}
                </ul>
                <Link to={plan.to} className={`block text-center py-3.5 rounded-xl font-bold text-sm transition ${plan.btn}`}>{plan.cta}</Link>
              </div>
            ))}
          </div>
        </div>
