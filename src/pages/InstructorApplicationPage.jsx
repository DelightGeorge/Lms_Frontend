...
// Replace the old uploadFile function with this:

const getUploadResourceType = (file) => {
  if (file.type.startsWith("video/")) return "video";
  if (file.type.startsWith("image/")) return "image";
  if (file.type === "application/pdf") return "image"; // Cloudinary handles PDFs via image endpoint
  return "raw"; // .doc, .docx, .zip, etc.
};
    ...
    <div className="space-y-1.5">
      ...
      {value ? (
        <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
          ...
          <p className="text-sm font-semibold text-emerald-700 truncate flex-1">
            File uploaded ✓
          </p>
          <button
            type="button"
        ...
        >
          {uploading ? (
            ...
            <Upload
              size={18}
              className="text-slate-400 group-hover:text-blue-500 shrink-0 transition"
            />
          )}
...
export default function InstructorApplicationPage() {
  ...
  const handleUploadId = async (file) => {
    setUploadingId(true);
    try {
      set("idDocumentUrl", await uploadFile(file));
    } catch {
      showToast("ID upload failed. Try again.", "error");
    } finally {
  ...
  const handleUploadCv = async (file) => {
    ...
    try {
      ...
      showToast("CV upload failed. Try again.", "error");
    } finally {
  ...
  const handleUploadVideo = async (file) => {
    ...
    try {
      set("sampleVideoUrl", await uploadFile(file, "lmspro/samples"));
    } catch {
      showToast("Video upload failed. Try again.", "error");
    } finally {
  ...
  const handleUploadPortfolio = async (file) => {
    ...
    try {
      set("portfolioUrl", await uploadFile(file));
    } catch {
      showToast("Portfolio upload failed. Try again.", "error");
    } finally {
          ...
          <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-12 text-center max-w-lg w-full">
            {existing.status === "APPROVED" ? (
              <>
                ...
                <p className="text-slate-500 mb-6">
                  Your application was approved. Head to your dashboard to start
                  creating courses.
                </p>
                <Link
                  to="/instructordashboard"
                  className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-8 py-3.5 rounded-xl transition inline-block"
                >
                  Go to Dashboard
                </Link>
              </>
              ...
              <>
                ...
                <p className="text-slate-500 mb-2">
                  Your application is under review. We typically respond within
                  2–3 business days.
                </p>
              ...
              <>
                ...
                {existing.rejectionReason && (
                  <p className="text-slate-600 mb-2 text-sm bg-red-50 rounded-xl p-3">
                    "{existing.rejectionReason}"
                  </p>
                )}
        ...
        <div className="min-h-screen flex items-center justify-center pt-20 px-4">
          <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-12 text-center max-w-lg w-full">
            ...
            <div className="space-y-3">
              <Link
                to="/"
                className="block bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-3.5 rounded-xl transition"
              >
                Back to Home
              </Link>
              <Link
                to="/courses"
                className="block border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold px-8 py-3.5 rounded-xl transition"
              >
                Browse Courses
              </Link>
            </div>
  ...
  return (
    <Layout>
      {toast && (
        <div
          className={`fixed top-6 right-6 z-[999] px-5 py-3.5 rounded-xl text-white font-bold shadow-2xl text-sm ${toast.type === "error" ? "bg-red-500" : "bg-emerald-500"}`}
        >
          {toast.msg}
        </div>
      )}
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-slate-300 text-sm">
                  Share your expertise and earn on LMSPRO
                </p>
              </div>
            </div>
              ...
              {STEPS.map((s, i) => {
                ...
                return (
                  <React.Fragment key={s.id}>
                    ...
                    {i < STEPS.length - 1 && (
                      <div
                        className={`flex-1 h-px ${done ? "bg-emerald-500/50" : "bg-white/10"}`}
                      />
                    )}
                  </React.Fragment>
                );
              ...
              <>
                <div>
                  ...
                  <p className="text-slate-500 text-sm">
                    Tell us who you are and what you'll be teaching
                  </p>
                </div>
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      This appears on your public instructor profile
                    </p>
                  </div>
                  <Textarea
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <input
                        ...
                        value={
                          form.portfolioUrl &&
                          !form.portfolioUrl.startsWith("http://res.cloudinary")
                            ? form.portfolioUrl
                            : ""
                        }
            ...
            {step === 2 && (
              <>
                <div>
                  ...
                  <p className="text-slate-500 text-sm">
                    What subjects do you specialise in and how long have you
                    been doing it?
                  </p>
                </div>
                <div className="space-y-4">
                  <div>
                    ...
                    />
                    <p className="text-xs text-slate-400 mt-1">
                      You can list multiple e.g. "Python, Machine Learning, Deep
                      Learning"
                    </p>
                  </div>
                  <div>
                    ...
                    >
                      ...
                      {[
                        ...
                        <option
                          key={y}
                          value={
                            typeof y === "string" && y.includes("+") ? 15 : y
                          }
                        >
                          {y}{" "}
                          {Number(String(y).replace("+", "")) === 1
                            ? "year"
                            : "years"}
                        </option>
                      ))}
                  ...
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-2">
                    ...
                    <p className="text-xs text-blue-700 leading-relaxed">
                      We recommend instructors have at least 2 years of
                      professional experience in their field to ensure quality
            ...
            {step === 3 && (
              <>
                <div>
                  ...
                  <p className="text-slate-500 text-sm">
                    We verify all instructors to protect our learner community
                  </p>
                </div>
                <div className="space-y-5">
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-2">
                    ...
                    <p className="text-xs text-amber-700 leading-relaxed">
                      All documents are securely stored and only used to verify
                      your identity. They are never shared publicly or with
                      third parties.
                    </p>
                  </div>
                </div>
            ...
            {step === 4 && (
              <>
                <div>
                  ...
                  <p className="text-slate-500 text-sm">
                    Help us understand why you want to teach on LMSPRO
                  </p>
                </div>
                <div className="space-y-4">
                  <Textarea
                    label="Why do you want to teach on LMSPRO?"
                    required
                    value={form.teachingMotivation}
                    onChange={(v) => set("teachingMotivation", v)}
                  ...
                  <div className="bg-slate-50 rounded-2xl border border-slate-100 p-5 space-y-3">
                    ...
                    {[
                      ...
                      [
                        "Sample Video",
                        form.sampleVideoUrl ? "✓ Uploaded" : "Not provided",
                      ],
                    ].map(([label, val]) => (
                      <div key={label} className="flex justify-between text-xs">
                        ...
                        <span
                          className={`font-bold ${val.includes("✓") ? "text-emerald-600" : val === "—" ? "text-red-400" : "text-slate-700"}`}
                        >
                          {val}
                        </span>
                      </div>
